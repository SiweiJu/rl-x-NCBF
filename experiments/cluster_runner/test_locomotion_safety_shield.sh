...
shield_on_count="$((robot_count * eta_count * gamma_count))"
shield_off_count="${robot_count}"
combo_count="$((shield_on_count + shield_off_count))"
task_id="${SLURM_ARRAY_TASK_ID:-0}"

if (( task_id >= combo_count )); then
    echo "SLURM_ARRAY_TASK_ID=${task_id} is outside the configured sweep size ${combo_count}." >&2
    echo "ROBOTS='${ROBOTS[*]}' ETA_CBFS='${ETA_CBFS[*]}' GAMMA_CS='${GAMMA_CS[*]}'" >&2
    exit 2
fi

if (( task_id < shield_on_count )); then
    safety_layer="${USE_SAFETY_LAYER:-True}"
    robot_index="$((task_id / (eta_count * gamma_count)))"
    ...
    off_task_id="$((task_id - shield_on_count))"
    robot_index="$((off_task_id % robot_count))"
    robot="${ROBOT:-${ROBOTS[robot_index]}}"
...
case "${robot}" in
    0|go2|unitree_go2)
        ...
        if [[ "${safety_tag}" == "off" ]]; then
            DEFAULT_RUN_NAME="go2_locomotion_sl_off"
        else
