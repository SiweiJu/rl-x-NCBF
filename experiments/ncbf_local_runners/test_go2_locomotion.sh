...
ROLLOUT_SAVE_NAME="${ROLLOUT_SAVE_NAME:-go2_locomotion_rollout}"
PROJECT_NAME="${PROJECT_NAME:-debug}"
EXP_NAME="${EXP_NAME:-go2_locomotion_test}"
MODEL_TAG="$(basename "${MODEL_PATH}")"
MODEL_TAG="${MODEL_TAG%.*}"
RUN_NAME="${RUN_NAME:-go2_locomotion_${MODEL_TAG}}"
...
conda run --no-capture-output -n "${CONDA_ENV}" python experiment.py \
    ...
    --environment.name="ncbf_mujoco.robot_locomotion.mujoco" \
