#!/usr/bin/env python3
"""
Evaluate safety prediction accuracy for early termination episodes.
...
For each PKL:
  - compute the accuracy for terminations (accurate predictions of early terminated episodes)
  - calculate false positives and false negatives
  - calculate predictions steps for true positives

Then write a CSV into the project folder:
  <project_folder>/pred_eval_summary_<timestamp>.csv
"""
...
def summarize_one_pkl(path: Path, threshold: int, prediction_threshold: float = 0.9) -> FileSummary:
    ...
    # Evaluate the prediction (Assuming the prediction is stored in `ep["predictions"]`)
    # predictions = np.array(ep["safe_prediction"])
    predictions = sigmoid(np.array(ep["predictions"])[:, 1, 0])
    accuracy = 0
    false_positives = 0
    false_negatives = 0
    prediction_steps_true_pos = []

    for i, ep in enumerate(rollouts):
        # Take last 5 predictions for evaluation
        last_5_preds = predictions[-5:] # Apply sigmoid

        # Check if the episode is finished early (e.g., less than threshold length)
        is_terminated_early = episode_length(ep) < threshold

        # Accuracy for terminations (check both conditions)
        if is_terminated_early and np.all(last_5_preds < 0.1):
            accuracy += 1
        elif not is_terminated_early and np.all(last_5_preds > 0.9):
            accuracy += 1

        # False positives (wrong prediction for terminated episodes)
        if is_terminated_early and not np.all(last_5_preds < 0.1):
            false_positives += 1

        # False negatives (wrong prediction for non-terminated episodes)
        if not is_terminated_early and np.any(last_5_preds < 0.1):
            false_negatives += 1

        # Predictions steps for true positives (early termination)
        # Predictions steps for true positives (early termination)
        if is_terminated_early:
            # Iterate through the episode's predictions (the full list, not just the last 5)
            for idx in reversed(range(min(len(predictions), 50))):
                pred = predictions[-idx] # Apply sigmoid to each prediction
                if pred < prediction_threshold:
                    prediction_steps_true_pos.append(idx)
                    break  # Exit as soon as we find the first correct prediction

    accuracy_rate = accuracy / n_eps if n_eps > 0 else float("nan")
    avg_prediction_steps = np.mean(prediction_steps_true_pos) if prediction_steps_true_pos else float("nan")
    ...
    return FileSummary(
        ...
        prediction_steps_true_pos=avg_prediction_steps,
        avg_return=avg_ret,
        std_return=std_ret,
...
def main():
    ...
    for proj, rows in sorted(by_project.items(), key=lambda kv: kv[0]):
        proj_path = Path(proj)
        out_path = proj_path / f"pred_eval_summary.csv"
        write_csv(rows, out_path)
        print(f"Wrote CSV: {out_path}  (n_files={len(rows)})")
    # Print the summary table for verification
    print("\nSummary Table:")
    print(
        f"{'Project':<25}{'Experiment':<20}{'Model':<30}{'Accuracy':<10}{'False Positives':<15}{'False Negatives':<15}")
    print("=" * 95)

    for row in summaries:
        print(
            f"{row.project:<25}{row.experiment:<20}{row.model:<30}{row.accuracy:<10.2f}{row.false_positives:<15}{row.false_negatives:<15}{row.prediction_steps_true_pos:<15}")

    print("\nSummary table printed successfully.")
