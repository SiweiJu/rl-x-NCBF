#!/usr/bin/env python3
"""
IEEEtran-style ONE figure with 8 subplots (2x4), but experiments may use DIFFERENT x-axes.
...
Row 1 (4 subplots): success rate vs chosen x (mean ± std over entry folders)
Row 2 (4 subplots): avg return  vs chosen x (mean ± std over entry folders)

Each CSV = one method (colored line + shaded band).

MODIFICATION IN THIS VERSION:
  - ONLY the left column has y-axis labels (and y tick labels).
    Other columns hide y-label and y tick labels.

Usage:
  python plot_ieee_2x4_lines_multi_x.py ours.csv base.csv \
    --labels Ours Baseline \
    --x_candidates sampling_prob action_noise obs_noise perturb_scale \
    --out ieee_2x4.png --no_show
...
def main():
    ...
    fig_h = 2.2
    fig, axes = plt.subplots(2, 4, figsize=(fig_w, fig_h), sharex=False)
    ...
    plt.subplots_adjust(left=0.06, right=0.995, top=0.88, bottom=0.2, wspace=0.1, hspace=0.08)
