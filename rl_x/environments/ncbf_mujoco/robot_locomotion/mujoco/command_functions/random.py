...
class RandomCommands:
    def __init__(self, env):
        ...
        self.velocity_ratio = np.array(self.env.env_config["command"]["velocity_ratio"])
    ...
    def get_next_command(self):
        ...
        max_command_velocity = self.env.internal_state["max_command_velocity"] * self.velocity_ratio

        goal_velocities = self.env.np_rng.uniform(size=(3,), low=-0.5 * max_command_velocity, high=max_command_velocity)
        ...
        # debug only
        goal_velocities[0] = np.abs(goal_velocities[0])
        goal_velocities[1] = 0
        goal_velocities[2] = 0
        print(goal_velocities)
