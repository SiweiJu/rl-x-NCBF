...
class G1BallReward(DefaultG1Reward):
    ...
    def extra_reward_terms(self, data, mjx_model, internal_state, action, info):
        if self.env.use_ball_plate:
            ...
            on_plate_reward = self.ball_plate_on_plate_coeff * metrics["on_plate"].astype(jnp.float32)
            ...
            ) * metrics["on_plate"].astype(jnp.float32)
            alive_reward = self.ball_plate_alive_coeff * (1.0 - metrics["dropped"].astype(jnp.float32))
            drop_penalty_reward = self.ball_plate_drop_penalty_coeff * -metrics["dropped"].astype(jnp.float32)
